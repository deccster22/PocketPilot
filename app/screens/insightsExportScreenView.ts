...
export type InsightsExportScreenViewData = {
  ...
  dispatchAvailabilityMessage: string | null;
  dispatchActionLabel: string | null;
  journalFollowThroughLabel: string | null;
  journalFollowThroughSummary: string | null;
...
function formatDispatchAvailabilityMessage(
  reason: Extract<ExportDispatchAvailability, { status: 'UNAVAILABLE' }>['reason'],
): string {
  switch (reason) {
    case 'DISPATCH_NOT_SUPPORTED':
      return 'This device cannot create this export yet, so the preview remains read-only.';
    case 'INSUFFICIENT_CONTENT':
      return 'There is not enough prepared reflection content to create this export yet.';
    default:
      return 'This export path is not enabled on this surface.';
  }
}

function formatDispatchResultMessage(
  result: PreparedExportDispatchResult | null | undefined,
): string | null {
  if (!result) {
    return null;
  }
  ...
  if (result.status === 'AVAILABLE') {
    return result.journalReferenceIncluded
      ? `Created ${result.fileLabel}. It keeps ${result.timezoneLabel} visible and includes the linked summary note.`
      : `Created ${result.fileLabel}. It keeps ${result.timezoneLabel} visible and stays with prepared reflection content only.`;
  }

  switch (result.reason) {
    case 'DISPATCH_FAILED':
      return 'PocketPilot could not create this export just now.';
    case 'DISPATCH_NOT_SUPPORTED':
      return 'This device cannot create this export yet.';
    case 'UNSUPPORTED_FORMAT':
      return 'This export format cannot be created from the current reflection context.';
    case 'INSUFFICIENT_CONTENT':
      return 'There is not enough prepared reflection content to create this export yet.';
    default:
      return 'Choose an export option before creating a file.';
  }
}
...
export function createInsightsExportScreenViewData(
  ...
  return {
    ...
    dispatchAvailabilityMessage:
      ...
      preparedExportRequestVM.availability.request.dispatchAvailability.status === 'UNAVAILABLE'
        ? formatDispatchAvailabilityMessage(
            preparedExportRequestVM.availability.request.dispatchAvailability.reason,
          )
        : null,
    dispatchActionLabel:
      preparedExportRequestVM.availability.status === 'AVAILABLE' &&
      preparedExportRequestVM.availability.request.dispatchAvailability.status === 'AVAILABLE'
        ? preparedExportRequestVM.availability.request.dispatchAvailability.canShare
          ? 'Create and share export'
          : 'Create export'
        : null,
    journalFollowThroughLabel:
      preparedExportRequestVM.availability.status === 'AVAILABLE' &&
      preparedExportRequestVM.availability.request.dispatchAvailability.status === 'AVAILABLE'
        ? preparedExportRequestVM.availability.request.dispatchAvailability.journalFollowThroughLabel
        : null,
    journalFollowThroughSummary:
      preparedExportRequestVM.availability.status === 'AVAILABLE' &&
      preparedExportRequestVM.availability.request.dispatchAvailability.status === 'AVAILABLE' &&
      preparedExportRequestVM.availability.request.dispatchAvailability.journalFollowThroughLabel
        ? preparedExportRequestVM.availability.request.journalReferenceIncluded
          ? 'The linked summary note will be added as a final PDF section.'
          : 'Optional. Add the linked summary note as a final PDF section when it helps.'
        : null,
    journalReferenceIncluded:
      preparedExportRequestVM.availability.status === 'AVAILABLE'
        ? preparedExportRequestVM.availability.request.journalReferenceIncluded
        : false,
    dispatchResultMessage: formatDispatchResultMessage(dispatchResult),
