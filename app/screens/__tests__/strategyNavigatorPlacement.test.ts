...
describe('strategy navigator placement', () => {
  it('keeps Strategy Preview as one top-level simulated surface with service-owned shaping', () => {
    ...
    expect(strategyNavigatorScreenSource).not.toMatch(
      /createStrategyNavigatorVM|createStrategyPreviewExplanation|selectStrategyPreviewKnowledge|fetchContextualKnowledgeAvailability|ContextualKnowledgeCard|knowledgeCatalog|listCatalog|strategyPreviewScenarios|signalCode|signalsTriggered|providerId|metadata|runtime|\border\b|\bbroker\b/,
    );
