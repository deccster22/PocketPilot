...
`P7-K16` merges compact Layer 3 example wiring across canonical strategy and concept docs plus the four payload-covered Trade Hub topics (`entry-price`, `stop-loss-price`, `target-price`, `reward-risk`), reuses existing canonical market-example/evidence docs where available, and records future/unmapped example slugs in `docs/knowledge/_meta/LAYER3_EXAMPLE_INTEGRATION_MAP.md`; runtime `knowledgeCatalog.ts` content is updated only for affected existing strategy/trade-hub topic IDs.
