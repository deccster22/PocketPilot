...
describe('createSinceLastCheckedVM', () => {
  it('is deterministic for identical inputs', () => {
    const params = {
      snapshot: createSnapshot(),
    };

    expect(createSinceLastCheckedVM(params)).toEqual(createSinceLastCheckedVM(params));
  });

  it('caps the briefing at three items and keeps account scope intact', () => {
    const result = createSinceLastCheckedVM({
      snapshot: createSnapshot(),
    });
  ...
  it('collapses cleanly when there is no meaningful change to surface', () => {
    const result = createSinceLastCheckedVM({
      snapshot: createSnapshot({
        orientationContext: {
          accountId: 'acct-1',
  ...
  it('returns no account context when the selected account cannot be resolved', () => {
    const result = createSinceLastCheckedVM({
      snapshot: createSnapshot({
        orientationContext: {
          currentState: {
  ...
  it('stays unavailable outside Snapshot', () => {
    expect(
